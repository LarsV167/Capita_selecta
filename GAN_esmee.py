...
class Discriminator(nn.Module):
    def __init__(self):
        super().__init__()
        self.label_emb = nn.Sequential(nn.Embedding(10, 50), nn.Linear(50,32*64*64)) #What should be the values here?
    ...
    def forward(self, img, seg): #input real or fake image and label 
       # x = torch.cat((img.view(img.size(0), -1),
        ...
        c = self.label_emb(torch.ShortTensor(seg.detach()))
        c = c.view(-1,32,64,64)
...
#        # x = torch.cat((img.view(img.size(0), -1),
#                           #seg.view(seg.size(0), -1)), dim=1)
#         c = self.label_emb(torch.LongTensor(img.detach()))
#         c = self.label_emb(torch.LongTensor(seg.detach())) 
#         x = torch.cat((c.detach(), img.detach()), dim=1)
#         #x = torch.cat((seg.detach(), img.detach()), dim=1)
#         x = x.view(-1, 32, 64, 64)
